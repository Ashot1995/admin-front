        ...
        >
            ...
            <template v-slot:top>
                <v-toolbar flat color="white">
                    <v-toolbar-title>Администраторы</v-toolbar-title>
                    <v-divider
                            class="mx-4"
                            inset
                            vertical
                    ></v-divider>
                    <v-spacer></v-spacer>
                    <v-btn color="green" dark large to="/admin/add">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            editItem(item) {
                this.$router.push({name: 'AdminEdit', params: {id: item.id}})
            },

            deleteItem(item) {
                let confirmation = confirm(`Администратор ${item.name} будет безвозвратно удален. Все равно удалить?`);
                if(confirmation) {
                    item.delete()
                        .then((promise) => {
                            this.getItems()
            ...
            getItems() {
                this.loading = true;
                Admin.index(this.queryParameters)
                    .then(data => {
